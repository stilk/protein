...
def GetCorrCountsForEachGene(ASType,  PSI_Threshold):
    OutDir = '/labs/ccurtis2/tilk/scripts/protein/Data/AS_Tables/' 
    AS = GetExpLevelsForASEvents(ASType, PSI_Threshold)
    AS['Bin'] = pd.cut(AS['MutLoad'], bins=[0,10,100,1000,30000], labels=[10,100,1000,30000])
    PerGene = AS.groupby(['Bin','GeneName','NMD']).size().unstack().reset_index().replace(np.nan,0).reset_index()
    PerGene['Fraction'] = PerGene[True]/(PerGene[True] + PerGene[False])
    Out = PerGene.groupby(['GeneName']).apply(GetCorCoef).reset_index()
    Out.to_csv(OutDir + 'TCGA_PerGeneTranscriptDepletionCorWithLoad_' + ASType + '_' + str(PSI_Threshold))
...
# GetCorrCountsForEachGene(ASType='RI', PSI_Threshold=thresh)
# GetCorrCountsForEachGene(ASType='ES', PSI_Threshold=thresh)
# GetCorrCountsForEachGene(ASType='AD', PSI_Threshold=thresh)
# GetCorrCountsForEachGene(ASType='AP', PSI_Threshold=thresh)
# GetCorrCountsForEachGene(ASType='AT', PSI_Threshold=thresh)
# GetCorrCountsForEachGene(ASType='AA', PSI_Threshold=thresh)
# GetCorrCountsForEachGene(ASType='ME', PSI_Threshold=thresh)
